...
## Open Source
Open source is cool if you're a programmer. You can study the code of any tool, figure out how it is working. It is also means that if something bugs you or software lacks something that you need, you can just send in a patch. In the closed-source world, you're stuck begging the vendor, or maybe if there's a plug-in system, trying to hack it in one way or another. There's open-source for other platforms, but while Windows is the "first class" citizen for games, Linux is decidedly the first class citizen for open source. If you're using an open-source package on Windows, chances are pretty good that it was ported from Linux and is secondary as a development target.
